...
class PANNS_ResNet15(nn.Module):

    emb_size = 2048

    def __init__(self, num_class, input_size, dropout=0.1, extract_embedding=True):
        super(PANNS_ResNet15, self).__init__()

        # Initialize the ResNet152 backbone without its final fully connected layer
        self.resnet = resnet152(pretrained=False, num_classes=0)
        self.resnet.fc = nn.Identity()  # Replace the final fc layer with an identity function to keep the feature map

        # Adjust the input channel size to match the provided input_size
        self.resnet.conv1 = nn.Conv2d(input_size, 64, kernel_size=7, stride=2, padding=3, bias=False)
