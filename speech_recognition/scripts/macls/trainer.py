...
from macls.models.panns import PANNS_CNN6, PANNS_CNN10
    ...
    def __train_epoch(self, epoch_id, local_rank, writer, nranks=0):
        ...
        for batch_id, (audio, label, input_lens_ratio) in enumerate(self.train_loader):
            ...
            if batch_id % self.configs.train_conf.log_interval == 0 and local_rank == 0:
                ...
                writer.add_scalars('Train_Test/Loss', {'Train': sum(loss_sum) / len(loss_sum)}, self.train_step)
                writer.add_scalars('Train_Test/Accuracy', {'Train': (sum(accuracies) / len(accuracies))},
                                   self.train_step)
                # 记录学习率
        ...
        # 将每个epoch的结果写入Excel文件
        df = pd.DataFrame(epoch_results)
        df.to_excel(f'train_results_epoch_{epoch_id}.xlsx', index=False)
    ...
    def train(self,
        ...
        for epoch_id in range(last_epoch, self.configs.train_conf.max_epoch):
            ...
            if local_rank == 0:
                ...
                logger.info('=' * 70)
                writer.add_scalars('Train_Test/Accuracy', {'Test': acc}, test_step)
                writer.add_scalars('Train_Test/Loss', {'Test': loss}, test_step)
                test_step += 1
                self.model.train()
                # # 保存最优模型
