...
class WholeBodyController {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('desired')) {
        ...
        this.desired = new WholeBodyState();
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    bufferOffset = WholeBodyState.serialize(obj.desired, buffer, bufferOffset);
    // Serialize message field [actual]
    bufferOffset = WholeBodyState.serialize(obj.actual, buffer, bufferOffset);
    // Serialize message field [error]
    bufferOffset = WholeBodyState.serialize(obj.error, buffer, bufferOffset);
    ...
    obj.command.forEach((val) => {
      bufferOffset = JointCommand.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static getMessageSize(object) {
    ...
    length += WholeBodyState.getMessageSize(object.desired);
    length += WholeBodyState.getMessageSize(object.actual);
    length += WholeBodyState.getMessageSize(object.error);
    object.command.forEach((val) => {
      length += JointCommand.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new WholeBodyController(null);
    ...
    if (msg.desired !== undefined) {
      resolved.desired = WholeBodyState.Resolve(msg.desired)
    }
    else {
      resolved.desired = new WholeBodyState()
    }
    ...
    if (msg.error !== undefined) {
      resolved.error = WholeBodyState.Resolve(msg.error)
    }
    else {
      resolved.error = new WholeBodyState()
    }

    if (msg.command !== undefined) {
      resolved.command = new Array(msg.command.length);
      for (let i = 0; i < resolved.command.length; ++i) {
        resolved.command[i] = JointCommand.Resolve(msg.command[i]);
      }
    }
    else {
      resolved.command = []
...
module.exports = WholeBodyController;
